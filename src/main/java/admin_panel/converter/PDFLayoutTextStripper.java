...
public class PDFLayoutTextStripper extends PDFTextStripper {
    ...
    private static final Logger LOG = Logger.getLogger(PDFLayoutTextStripper.class);
...
class TextLine {
    ...
    private static final char SEPARATE_CHARACTER = ' ';
    ...
    public void writeCharacterAtIndex(final Character character) {
        ...
        if ( this.indexIsInBounds(index) && this.line.charAt(index) == SEPARATE_CHARACTER) {
    ...
    private int computeIndexForCharacter(final Character character) {
        ...
        if ( !this.indexIsInBounds(index) ) {
            ...
            if ( isCharacterPartOfPreviousWord && !isCharacterAtTheBeginningOfNewLine ) {
                ...
                if ( this.line.charAt(index) != SEPARATE_CHARACTER) {
                    index = index + 1;
                } else {
    ...
    private boolean isSpaceCharacterAtIndex(int index) {
        return this.line.charAt(index) != SEPARATE_CHARACTER;
    }
    ...
    private int findMinimumIndexWithSpaceCharacterFromIndex(int index) {
        int newIndex = index;
        while( newIndex >= 0 && this.line.charAt(newIndex) == SEPARATE_CHARACTER) {
            newIndex = newIndex - 1;
        }
        return newIndex + 1;
    }
